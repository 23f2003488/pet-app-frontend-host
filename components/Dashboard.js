const Dashboard = {
  template: `
    <!-- CHANGE 3: The container's style is now bound to a dynamic property -->
  ...
  data() {
    return {
      ...
      // CHANGE 4: New data property for the background
      dashboardBackground: 'assets/img/backgrounds/dash-bg.jpeg', // Default background
      loading: true
    };
  },
  methods: {
    async handleLogout() {
      // Also clear the equipped gift on logout
      localStorage.removeItem('equippedGiftId');
      localStorage.removeItem('equippedGiftUrl');
  ...
  mounted() {
    // Check for equipped items when the dashboard loads
    const equippedCostumeUrl = localStorage.getItem('equippedCostumeUrl');
    const equippedGiftUrl = localStorage.getItem('equippedGiftUrl');

    if (equippedCostumeUrl) {
      this.petImage = equippedCostumeUrl;
    }
    if (equippedGiftUrl) {
      this.dashboardBackground = equippedGiftUrl;
    }
    ...
    // CHANGE 5: Removed the static background-image from the CSS
    style.textContent = `
      ...
      .pet-image { position: absolute; top: 67%; left: 50%; transform: translate(-50%, -50%); width: 200px; height: auto; z-index: 1; animation: float 2.5s ease-in-out infinite; }
      @keyframes float { 0% { transform: translate(-50%, -50%) scale(1); } 50% { transform: translate(-50%, -52%) scale(1.03); } 100% { transform: translate(-50%, -50%) scale(1); } }
    `;
