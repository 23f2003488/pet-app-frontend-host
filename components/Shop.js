          ...
          <div class="scroll-container">
            ...
            <div class="items-wrapper" ref="wrappers">
              <div class="item-card" v-for="(item, i) in category.items" :key="i">
                ...
                <div class="price-box">
                  <span v-if="item.category === 'Gifts'" class="xp-price">XP: {{ item.xp_price }}</span>
                ...
                <div class="button-container">
                  ...
                  <div v-else-if="item.category === 'Costumes' || item.category === 'Gifts'">
                    <button v-if="item.quantity === 0" class="shop-button" :disabled="buyingItemId === item.id" @click="buyItem(item)">
                      <span v-if="buyingItemId === item.id">...</span><span v-else>Buy</span>
                    </button>
                    <div v-else>
  ...
  methods: {
    async fetchShopItems() {
      try {
        ...
        const equippedGiftId = localStorage.getItem('equippedGiftId');

        this.categories = Object.keys(shopData).map(categoryName => ({
          name: categoryName,
          items: shopData[categoryName].map(item => ({
            ...item,
            quantity: inventoryMap[item.id]?.quantity || 0,
            inventory_id: inventoryMap[item.id]?.inventory_id,
            is_equipped: item.id === equippedCostumeId || item.id === equippedGiftId
          }))
        }));
      } catch (err) {
    ...
    // CHANGE: This method is now fixed
    async equipItem(item) {
      // When equipping a Costume, ONLY affect costume keys
      ...
      // When equipping a Gift, ONLY affect gift keys
      else if (item.category === 'Gifts') {
        localStorage.setItem('equippedGiftId', item.id);
        localStorage.setItem('equippedGiftUrl', item.image_url);
      }
